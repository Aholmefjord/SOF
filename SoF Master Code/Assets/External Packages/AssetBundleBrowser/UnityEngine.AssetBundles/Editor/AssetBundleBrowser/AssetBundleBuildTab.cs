    ...
    public class AssetBundleBuildTab
    {
        ...
        public void OnEnable(Rect pos, EditorWindow parent)
        {
            ...
            m_CopyToStreaming = new ToggleData(
                ...
                m_OnToggles);
            m_RebuildScript = new ToggleData(
                false,
                "Rebuild Scripts",
                "Rebuild script for when compiler complains when script signature is different from assetbundles",
                m_OnToggles);
        ...
        public void OnGUI(Rect pos)
        {
            ...
            using (new EditorGUI.DisabledScope (!AssetBundleModel.Model.DataSource.CanSpecifyBuildOutputDirectory)) {
                ...
                newState = GUILayout.Toggle(
                    m_RebuildScript.state,
                    m_RebuildScript.content);
                if (newState != m_RebuildScript.state) {
                    if (newState)
                        m_OnToggles.Add(m_RebuildScript.content.text);
                    else
                        m_OnToggles.Remove(m_RebuildScript.content.text);
                    m_RebuildScript.state = newState;
                }
            }
        ...
        private void ExecuteBuild()
        {
            ...
            if (m_RebuildScript.state) {
                BuildPlayerOptions options = new BuildPlayerOptions();
                options.locationPathName = System.Environment.CurrentDirectory + "/Build/sof.apk";
                options.target = (BuildTarget)m_BuildTarget;
                options.options = BuildOptions.BuildScriptsOnly;
                Debug.LogFormat("Build script result: {0}", BuildPipeline.BuildPlayer(options));
            }
