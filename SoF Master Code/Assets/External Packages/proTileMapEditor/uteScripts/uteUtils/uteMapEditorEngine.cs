...
public class uteMapEditorEngine : MonoBehaviour {
	...
	public IEnumerator uteInitMapEditorEngine()
	{
		...
		//Create start point marker (PY)
		Vector3 startPos = new Vector3(500.5f,0,500.5f);
		GameObject StartParticle = (GameObject) Instantiate((GameObject)Resources.Load("uteForEditor/uteStartMarker"),startPos,Quaternion.identity);
	...
	public List<GameObject> createColliderToObject(GameObject obj, GameObject obj_rs)
	{
		...
		Collider cldrs = (Collider)obj.GetComponent<Collider>();
		...
		//sets it to built in collider if it is found
		if(cldrs)
		{
			lowestPointX = cldrs.bounds.min.x;
			lowestPointY = cldrs.bounds.min.y;
			lowestPointZ = cldrs.bounds.min.z;
			highestPointX = cldrs.bounds.max.x;
			highestPointY = cldrs.bounds.max.y;
			highestPointZ = cldrs.bounds.max.z;
		}
