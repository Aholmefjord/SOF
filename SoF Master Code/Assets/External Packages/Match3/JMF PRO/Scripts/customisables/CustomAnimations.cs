...
public enum animType{GLOBALDESTROY, NOMOREMOVES, ARROWH, ARROWV, ARROWVH, ARROWTX, STAR, RAINBOW, BOMB,
...
public class CustomAnimations : MonoBehaviour {
	...
	public void doAnim(animType animType, int x, int y){
		...
		case animType.ARROWTLBR : // is when match-4 power TopLeft <> BottomRight
			if(TLBRAnim != null){ // animation effect
				if(JMFUtils.isPooling){
					PoolManager.Pools[animPoolName].Spawn(TLBRAnim.transform,gm.board[x,y].position,Quaternion.identity);
				} else {
		...
		case animType.ARROWTRBL : // is when match-4 power TopLeft <> BottomRight
			if(TRBLAnim != null){ // animation effect
				if(JMFUtils.isPooling){
					PoolManager.Pools[animPoolName].Spawn(TRBLAnim.transform,gm.board[x,y].position,Quaternion.identity);
				} else {
