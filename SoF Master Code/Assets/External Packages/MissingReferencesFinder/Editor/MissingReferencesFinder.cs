...
public class MissingReferencesFinder : MonoBehaviour 
{
	...
	private static void FindMissingReferences(string context, GameObject[] objects)
	{
		...
		foreach (var go in objects)
		{
			...
			foreach (var c in components)
			{
				...
				{
                    counter++;
					Debug.LogError("["+counter+"] Missing Component in GO: " + GetFullPath(go), go);
					continue;
				}
				...
				while (sp.NextVisible(true))
				{
					if (sp.propertyType == SerializedPropertyType.ObjectReference)
					{
						...
						{
							ShowError(++counter, context, go, c.GetType().Name, ObjectNames.NicifyVariableName(sp.name));
						}
					}
				}
			}
	...
	private static void ShowError (int counter, string context, GameObject go, string componentName, string propertyName)
	{
		var ERROR_TEMPLATE = "[{4}] Missing Ref in: [{3}]{0}. Component: {1}, Property: {2}";

		Debug.LogError(string.Format(ERROR_TEMPLATE, GetFullPath(go), componentName, propertyName, context, counter), go);
	}
