...
public class MediaPlayerCtrl : MonoBehaviour
{
    ...
    private float LastVol;// to save previous volume
        ...
        if (m_TargetMaterial != null)
        {
            ...
            for (int iIndex = 0; iIndex < m_TargetMaterial.Length; iIndex++)
            {
                if (m_TargetMaterial[iIndex] != null)
                {
                    if (m_TargetMaterial[iIndex].GetComponent<RawImage>() != null)
                    {
                        m_TargetMaterial[iIndex].GetComponent<RawImage>().uvRect = new Rect(0, 0, 1, 1);
                    }
                }
    ...
    void OnDisable()
    {
        ...
        {
            Pause();
            AudioSystem.set_bgm_volume(1);
        }
    }
    public void EnableBGM()
    {
        AudioSystem.set_bgm_volume(LastVol);
    }
    ...
    void OnEnable()
    {
        ...
        {
            if (PlayerPrefs.GetInt("IsYoutube") == 1)
            {
                //YoutubeVideo.Instance.RequestVideo(PlayerPrefs.GetString("EpisodeToPlay"), 360)
                Load(PlayerPrefs.GetString("EpisodeToPlay"));
            } else
            {
                Load(PlayerPrefs.GetString("EpisodeToPlay"));
            }
        ...
        //       Load(@"http://youtu.be/uBO_366ngIc");
    ...
    void Update()
    {
        m_TargetMaterial[0].SetActive((GetCurrentState() == MEDIAPLAYER_STATE.READY || GetCurrentState() == MEDIAPLAYER_STATE.PAUSED || GetCurrentState() == MEDIAPLAYER_STATE.PLAYING));
    ...
    public void Play()
    {
        ...
        if (m_CurrentState == MEDIAPLAYER_STATE.PAUSED)
        {
            m_TargetMaterial[0].SetActive(true);
            Call_RePlay();
        }
    ...
    public void Stop()
    {
        ...
        m_TargetMaterial[0].SetActive(false);
        if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING)
            Call_Pause();
    ...
    public void TooglePauseResume()
    {
        ...
        else if (m_CurrentState == MEDIAPLAYER_STATE.PAUSED)
        {
            Call_RePlay();
    ...
    {
        LastVol = AudioSystem.get_bgm_volume();
            AudioSystem.set_bgm_volume(0);
    ...
    /*
                        Transform test = m_TargetMaterial[i].rectTransform();
    
                        Debug.Log("Media Video transform y: " + test.localScale.y);

                        if(test.localScale.y < 0)
                            m_TargetMaterial[i].rectTransform().localScale = new Vector3(test.localScale.x, -test.localScale.y, test.localScale.z);
    */
					}
				}
                    ...
                    for (int i = 0; i < m_TargetMaterial.Length; i++)
                    {
                        ...
                        if (m_TargetMaterial[i].GetComponent<RawImage>() != null)
                        {
                            if (m_TargetMaterial[i].GetComponent<RawImage>().texture != m_VideoTexture)
                            {
                                ...
                                /*
                                Transform test = m_TargetMaterial[i].rectTransform();
                                m_TargetMaterial[i].rectTransform().localScale = new Vector3(test.localScale.x, 0.6f, test.localScale.z);
                                test = m_TargetMaterial[i].rectTransform();
                                */
                            }
                        }
                    }
