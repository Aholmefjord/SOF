/****************************************
 * MD5 Hash algorithm
 * Implement the MD5 hash algorithm described at https://tools.ietf.org/html/rfc1321 without using System.Security.Cryptography namespace
 * Usage:
 *  - Static methods: use MD5's static methods to hash/compare value as either string or byte[]
 *    Recommended for quick, check-and-forget evaluations
 *  - Manual cache object: instantiate an MD5 object to hash/compare value
 *    Recommended for repeatitive and heavy hash/check operations
 *
 ***************************************/
...
public class HashAlgorithm {
	...
	private static HashAlgorithm Instance {
		get {
			if (_instance == null)
	...
	public static string Hash (byte[] value) {
		Instance.ValueAsByte = value;
		return Instance.HashValue;
	}

	public static bool CheckHashValue (string obj, string hash) {
		return Hash (obj).ToUpper () == hash.ToUpper ();
	}

	public static bool CheckHashValue (byte[] obj, string hash) {
		return Hash (obj).ToUpper () == hash.ToUpper ();
	}
	...
	public HashAlgorithm (byte[] value) {
		ValueAsByte = value;
	}
	/******************************************************************************/
	/*********************METHODS**************************/
	...
	protected void TransF (ref uint a, uint b, uint c, uint d, uint k, ushort s, uint i) {
		a = b + HashHelper.RotateLeft ((a + ((b & c) | (~(b) & d)) + X[k] + T[i - 1]), s);
	}
	...
	protected void TransG (ref uint a, uint b, uint c, uint d, uint k, ushort s, uint i) {
		a = b + HashHelper.RotateLeft ((a + ((b & d) | (c & ~d)) + X[k] + T[i - 1]), s);
	}
	...
	protected void TransH (ref uint a, uint b, uint c, uint d, uint k, ushort s, uint i) {
		a = b + HashHelper.RotateLeft ((a + (b ^ c ^ d) + X[k] + T[i - 1]), s);
	}
	...
	protected void TransI (ref uint a, uint b, uint c, uint d, uint k, ushort s, uint i) {
		a = b + HashHelper.RotateLeft ((a + (c ^ (b | ~d)) + X[k] + T[i - 1]), s);
	}
	...
	/// constants for md5
	/// </summary>
	protected enum MD5InitializerConstant : uint {
		A = 0x67452301,
		B = 0xEFCDAB89,
		C = 0x98BADCFE,
		D = 0X10325476
	...
	sealed protected class Digest {
		...
		public override string ToString () {
			string st;
			st = HashHelper.ReverseByte (A).ToString ("X8") +
				HashHelper.ReverseByte (B).ToString ("X8") +
				HashHelper.ReverseByte (C).ToString ("X8") +
				HashHelper.ReverseByte (D).ToString ("X8");
			return st;
		}
	}
	...
	/// </summary>
	sealed protected class HashHelper {
		private HashHelper () { }
		/// <summary>
