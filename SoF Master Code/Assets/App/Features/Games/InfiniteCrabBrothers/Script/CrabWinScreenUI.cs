...
public class CrabWinScreenUI : BaseUI
{

    public static CrabWinScreenUI Current { get; private set; }
	...
	public static void Open(bool gameWon, int TotalScore = 20000, int goldEarned = 0, int rGemEarned = 0, int uGemEarned = 0, int stars = 0, bool loadBuddy = false)
    {
        if (Current != null)
        { 
            Current.Close();
        }
        //GameObject go = CachedResources.Spawn("ui/Crab Win Screen", false);
        CachedResources.Spawn(Constants.CRAB_BROS_SHARED, "Crab Win Screen", (result) => {
            GameObject go = result;

            go.transform.SetParent(UIRoot.transform);
            Current = go.GetComponent<CrabWinScreenUI>();
            Current.gameWon = gameWon;
            Current.TotalScore = TotalScore;
            Current.HighestCombo = stars;
            Current.Highest = GameState.infiniteProg.GetLevel(GameState.infiniteProg.selectedLevel).pointsEarned;
    ...
    public void SetupUI()
    {
        MultiLanguage.getInstance().apply(gameObject.FindChild("Total Score Text"), "win_screen_total_score");
        MultiLanguage.getInstance().apply(gameObject.FindChild("Highest Combo Text"), "manta_win_screen_highest_combo");
        MultiLanguage.getInstance().apply(gameObject.FindChild("Highest Score"), "win_screen_highest_score");
        MultiLanguage.getInstance().apply(gameObject.FindChild("Rewards Text"), "win_screen_rewards");
        MultiLanguage.getInstance().apply(gameObject.FindChild("Lose game Text"), "win_screen_lose_game_text");
    }
	...
	protected override void Start()
    {
		...
		uiManager["Back Button"].OnClick(() =>
        {
            GameState.me.Upload();
            Close();
            if (GameState.infiniteProg.selectedLevel == 9 && Constants.uses_cinematics)
            {
                GoToCinematic();
            }
            else
            {
                //UnityEngine.SceneManagement.SceneManager.UnloadScene(Application.loadedLevel);
                //UnityEngine.SceneManagement.SceneManager.LoadScene("crab_stage_selected");
                MainNavigationController.DoAssetBundleLoadLevel(Constants.CRAB_BROS_SCENES, "crab_stage_selected");
            }
     //       CrabLevelSelectUI.Open();
        });
        ...
        uiManager["Replay Button"].OnClick(() =>
        {
            GameState.me.Upload();
            Close();
            if (GameState.infiniteProg.selectedLevel == 9 && Constants.uses_cinematics && Constants.uses_cinematics)
            {
                GoToCinematic();
            }
            else
            {
                UnityEngine.SceneManagement.SceneManager.UnloadScene("inifinite_crab_brothers_game");
                //UnityEngine.SceneManagement.SceneManager.LoadScene("inifinite_crab_brothers_game");
        ...
        uiManager["Next Button"].OnClick(() =>
        {
            ...
            else if (GameState.infiniteProg.selectedLevel >= gameState.endLevel)
            {
                //Goto level select.
                UnityEngine.SceneManagement.SceneManager.UnloadScene(Application.loadedLevel);
                //UnityEngine.SceneManagement.SceneManager.LoadScene("crab_stage_selected");
            ...
            {
                GameState.infiniteProg.selectedLevel++;
                UnityEngine.SceneManagement.SceneManager.UnloadScene("inifinite_crab_brothers_game");
                //UnityEngine.SceneManagement.SceneManager.LoadScene("inifinite_crab_brothers_game");
                //MainNavigationController.GoToScene("inifinite_crab_brothers_game");
                MainNavigationController.DoAssetBundleLoadLevel(Constants.CRAB_BROS_SCENES, "inifinite_crab_brothers_game");
        ...
        if (gameWon)
        {
            AnalyticsSys.Instance.Report(REPORTING_TYPE.GameResult, "CrabbyPatty Won Level:" + (GameState.infiniteProg.selectedLevel + 1).ToString() + " Stars:" + Stars.ToString()/* + " Score:" + TotalScore.ToString()*/);
            DOTween.Sequence()
...
//                        }
				...
				.Append(uiManager["Rewards"].GetComponent<CanvasGroup>().DOFade(1f, 0.3f))
		...
		{
            AnalyticsSys.Instance.Report(REPORTING_TYPE.GameResult, "CrabbyPatty Lose Level:" + (GameState.infiniteProg.selectedLevel + 1).ToString());
            uiManager["Back Button"].SetActive(true);
			uiManager["Replay Button"].SetActive(true);
			uiManager["Next Button"].SetActive(false);
