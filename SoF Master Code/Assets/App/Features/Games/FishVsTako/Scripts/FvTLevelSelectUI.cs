...
namespace FvTGame
{
    public class FvTLevelSelectUI : BaseUI
    {
        ...
        public static void Open()
        {
            ...
            BlankScreenUI.Open();
            thisGO = CachedResources.Spawn("ui/FvT Level Select", false);
        ...
        private void Update()
        {
            ...
            {// GameState.tsumProg.levelCount) {
                displayLevels = GameState.FvTProg.levelCount;
                uiManager["Vert Layout"].Clear();
                currentPage = 0;
            }

            var totalPage = 3;
            var pageBookmarks = new float[totalPage];
            
            if (!hasFinished)
            {
                hasFinished = true;
                for (var i = 0; i < 30/*mGameState.levelCount*/; ++i)
                {
                    var currentLevel = GameState.FvTProg.GetLevel(mGameState.startLevel + i - 1);
                    ...
                    //HACK
                    //currentLevel.status = FvTProgression.ELevelStatus.Available;

                    if (currentLevel.status == FvTProgression.ELevelStatus.Locked)
                    {
                        stageUI["Locked"].SetActive(true);
                        stageUI["New"].SetActive(false);
                        stageUI["Cleared"].SetActive(false);
                    }
                    else if (currentLevel.status == FvTProgression.ELevelStatus.Available)
                    {
                        ...
                        stageUI["New"].OnClick(() => SelectLevel(currentIndex));
                    }
                    else
                    {
                        stageUI["Locked"].SetActive(false);
                        ...
                        stageUI["Cleared"].OnClick(() => SelectLevel(currentIndex));
            ...
            uiManager["Right Arrow"].SetActive(currentPage < totalPage-1);
            uiManager["Right Arrow"].OnClick(() => { ++currentPage; hasFinished = false; });
        }

        public void SelectLevel(int levelId)
        {
            //GameState.FvTProg = new FvTProgression();
            GameState.FvTProg.selectedLevel = levelId;
