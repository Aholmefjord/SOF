...
using JULESTech;
public class FvTWinScreen : BaseUI
{
    public void GoToCinematic()
    {
        CutsceneDialogueController.isPartTwo = true;
        MainNavigationController.GoToScene("Cinematics_chpt4");
    }
    public static FvTWinScreen Current { get; private set; }
    ...
    public static void Open(bool gameWon = true, int TotalScore = 20000, int HighestCombo = 8, int goldEarned = 0, int rGemEarned = 0, int uGemEarned = 0, bool loadBuddy = false)
    {
        ...
        //thisGO = CachedResources.Spawn("Games/FvT/Prefab/FvT Results Screen", false);
        CachedResources.Spawn(Constants.FVT_SHARED, "FvT Results Screen", (resultObject) =>
        {
            thisGO = resultObject;
            //thisGO.transform.SetParent(UIRoot.transform);
            Current = thisGO.GetComponent<FvTWinScreen>();
            Current.GameWon = gameWon;
            Current.TotalScore = TotalScore;
            Current.HighestCombo = HighestCombo;
            Current.Highest = 1000;// GameState.tangramProg.GetLevel(GameState.tangramProg.selectedLevel).pointsEarned;
    ...
    protected override void Start()
    {
        ...
        uiManager["Next Button"].OnClick(() => {
            ...
            GameState.FvTProg.selectedLevel = GameState.FvTProg.selectedLevel + 1;
            Close();
            //SceneManager.LoadScene("FvTGameScene");
            MainNavigationController.DoAssetBundleLoadLevel(Constants.FVT_SCENES, "FvTGameScene");
        });
