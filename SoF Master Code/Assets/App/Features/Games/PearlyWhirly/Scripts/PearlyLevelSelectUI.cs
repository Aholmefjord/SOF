...
public class PearlyLevelSelectUI : BaseUI
{
    public static PearlyLevelSelectUI Current { get; private set; }
    ...
    public static void Open()
    {
		...
		BlankScreenUI.Open();
        //thisGO = CachedResources.Spawn("ui/Pearly Level Select", false);
        CachedResources.Spawn(Constants.PEARLY_SHARED, "Pearly Level Select", (loadedAsset) => {
            thisGO = GameObject.Instantiate(loadedAsset as GameObject, UIRoot.transform);
            Current = thisGO.GetComponent<PearlyLevelSelectUI>();
        });
    }
    ...
    protected override void Start()
    {
        ...
        uiManager["Critter Level Image"].OnClick(() => {
            ...
            MainNavigationController.DoAssetBundleLoadLevel(Constants.PEARLY_SCENES, "Level Complete");
            Close();
            /*DOTween.Sequence()
                .AppendInterval(1f)
    ...
    private void Update()
    {
        ...
        if (displayLevels == -1)
        {
            displayLevels = mGameState.levelCount;//GameState.pearlyProg.levelCount;
        }
        else if (displayLevels != mGameState.levelCount)//GameState.pearlyProg.levelCount)
        {
            displayLevels = mGameState.levelCount;// GameState.pearlyProg.levelCount;
        ...
        if (!hasFinished)
        {
            ...
            {
                var currentLevel = GameState.pearlyProg.GetLevel(mGameState.startLevel + i - 1);
    ...
    {
        GameState.pearlyProg.selectedLevel = levelId;
        //SceneManager.LoadScene("pearly");
