...
Shader "UnityCoder/GradientGlow2" {
	...
	//        _ToonShade ("ToonShader Cubemap(RGB)", CUBE) = "" { Texgen CubeNormal }
		...
		CGINCLUDE
		// Upgrade WCTE: excluded shader from DX11 and Xbox360; has structs without semantics (struct v2f members vpos)
#pragma exclude_renderers d3d11 xbox360
		// Upgrade WCTE: excluded shader from Xbox360; has structs without semantics (struct v2f members vpos)
#pragma exclude_renderers xbox360
#include "UnityCG.cginc"

	struct appdata {
		float4 vertex : POSITION;
		float3 normal : WCRMAL;
	};
