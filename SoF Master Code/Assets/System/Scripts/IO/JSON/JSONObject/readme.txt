...
= Usage =
Users should not have to modify the JSOWCbject class themselves, and must follow the very simple proceedures outlined below:
...
*Copy to new JSOWCbject
*Merge with another JSOWCbject (experimental)
...
JSOWCbject j = new JSOWCbject(JSOWCbject.Type.OBJECT);
//number
j.AddField("field1", 0.5);
//string
j.AddField("field2", "sampletext");
//array
JSOWCbject arr = new JSOWCbject(JSOWCbject.Type.ARRAY);
...
NEW! The constructor, Add, and AddField functions now support a nested delegate structure.  This is useful if you need to create a nested JSOWCbject in a single line.  For example:
<syntaxhighlight lang="csharp">
DoRequest(URL, new JSOWCbject(delegate(JSOWCbject request) {
	request.AddField("sort", delegate(JSOWCbject sort) {
		sort.AddField("_timestamp", "desc");
	});
	request.AddField("query", new JSOWCbject(delegate(JSOWCbject query) {
		query.AddField("match_all", JSOWCbject.obj);
	}));
	request.AddField("fields", delegate(JSOWCbject fields) {
...
== Decoding ==
Decoding is much simpler on the input end, and again, what you do with the JSOWCbject will vary on a per-project basis.  One of the more complicated way to extract the data is with a recursive function, as drafted below.  Calling the constructor with a properly formatted JSON string will return the root object (or array) containing all of its children, in one neat reference!  The data is in a public ArrayList called list, with a matching key list (called keys!) if the root is an Object.  If that's confusing, take a glance over the following code and the print() method in the JSOWCBject class.  If there is an error in the JSON formatting (or if there's an error with my code!) the debug console will read "improper JSON formatting".


<syntaxhighlight lang="csharp">
string encodedString = "{\"field1\": 0.5,\"field2\": \"sampletext\",\"field3\": [1,2,3]}";
JSOWCbject j = new JSOWCbject(encodedString);
accessData(j);
//access data (and print it)
void accessData(JSOWCbject obj){
	switch(obj.type){
		case JSOWCbject.Type.OBJECT:
			for(int i = 0; i < obj.list.Count; i++){
				string key = (string)obj.keys[i];
				JSOWCbject j = (JSOWCbject)obj.list[i];
				Debug.Log(key);
				accessData(j);
			}
			break;
		case JSOWCbject.Type.ARRAY:
			foreach(JSOWCbject j in obj.list){
				accessData(j);
			}
			break;
		case JSOWCbject.Type.STRING:
			Debug.Log(obj.str);
			break;
		case JSOWCbject.Type.NUMBER:
			Debug.Log(obj.n);
			break;
		case JSOWCbject.Type.BOOL:
			Debug.Log(obj.b);
			break;
		case JSOWCbject.Type.NULL:
			Debug.Log("NULL");
			break;
...
<syntaxhighlight lang="csharp">
new JSOWCbject(data);
list.GetField("hits", delegate(JSOWCbject hits) {
	hits.GetField("hits", delegate(JSOWCbject hits2) {
		foreach (JSOWCbject gameSession in hits2.list) {
